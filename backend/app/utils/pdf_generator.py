...
class FacultyCV(FPDF):
    def header(self):
        ...
        self.cell(0, 10, "Curriculum Vitae", border=0, align="C", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        self.ln(15)
...
def generate_pdf_cv(profile, education, experience, publications, projects) -> bytes:
    ...
    pdf.cell(0, 10, f"{profile.first_name} {profile.last_name}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    if profile.phone_number:
        pdf.cell(0, 8, f"Phone: {profile.phone_number}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(5)
    ...
    if education:
        ...
        for edu in education:
            ...
            pdf.set_text_color(44, 62, 80)
            pdf.cell(0, 6, f"{edu.degree} in {edu.field_of_study}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            
            pdf.set_font("helvetica", "", 11)
            pdf.set_text_color(0, 0, 0)
            date_str = f"{edu.start_date.strftime('%Y')} - {edu.end_date.strftime('%Y') if edu.end_date else 'Present'}"
            pdf.cell(0, 6, f"{edu.institution} | {date_str}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.ln(4)
    ...
    if experience:
        ...
        for exp in experience:
            pdf.set_font("helvetica", "B", 12)
            pdf.set_text_color(44, 62, 80)
            pdf.cell(0, 6, f"{exp.job_title} at {exp.organization}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
