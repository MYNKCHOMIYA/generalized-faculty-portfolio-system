...
def create_education(*, session: SessionDep, edu_in: EducationCreate, current_user: User = Depends(get_current_active_user)) -> Any:
...
def read_my_education(session: SessionDep, current_user: User = Depends(get_current_active_user)) -> Any:
    profile = get_current_faculty_profile(session, current_user.id)
    return session.query(Education).filter(Education.faculty_id == profile.id).all()

@router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT, response_class=Response)
def delete_education(*, session: SessionDep, id: str, current_user: User = Depends(get_current_active_user)) -> None:
    profile = get_current_faculty_profile(session, current_user.id)
    record = session.query(Education).filter(Education.id == id, Education.faculty_id == profile.id).first()
    if not record:
