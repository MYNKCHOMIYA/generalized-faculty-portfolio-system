...
from app.schemas.faculty_profile import FacultyProfileCreate, FacultyProfileUpdate, FacultyProfileResponse
from fastapi import UploadFile, File
from app.utils.cloudinary_handler import upload_image

router = APIRouter()

@router.post("/", response_model=FacultyProfileResponse, status_code=status.HTTP_201_CREATED)
def create_profile(
    *,
    session: SessionDep,
    ...
    existing_profile = session.query(FacultyProfile).filter(FacultyProfile.user_id == current_user.id).first()
    if existing_profile:
        raise HTTPException(
            status_code=400,
...
def read_profile_me(
    ...
    """
    profile = session.query(FacultyProfile).filter(FacultyProfile.user_id == current_user.id).first()
    if not profile:
...
def upload_profile_image(
    *,
    ...
    profile = session.query(FacultyProfile).filter(FacultyProfile.user_id == current_user.id).first()
    if not profile:
        raise HTTPException(status_code=404, detail="Faculty profile not found. Please create one first.")
